...
export default function Home() {
  ...
  const runSession = async (index: number, currentPhase: "jp" | "en", rep: number) => {
    ...
    // Increment completed reps after playback ends
    setCompletedRep(rep);
    ...
    if (rep < maxReps) {
      ...
      await wait(800); // Wait to see the phase bar reach 100%
      if (!isPlayingRef.current) return;
      setCompletedRep(0); // Reset for EN phase concurrently
      runSession(index, "en", 1);
    } else {
      // Current sentence's EN phase (all repetitions) finished
      await wait(800); // Wait to see the phase bar reach 100%
      
      // Update both concurrently to sync animations
      setCompletedIndex(index + 1);
      setCompletedRep(0);
      
      if (index + 1 < sentencePairs.length) {
        await wait(800); // Delay before starting next sentence
        runSession(index + 1, "jp", 1);
      } else {
        await wait(1000); // Final pause to see the 100% bars
        if (isPlayingRef.current) {
          setIsPlaying(false);
          isPlayingRef.current = false;
          // Reset progress bars on finish
          setCompletedIndex(0);
          setCompletedRep(0);
          setCurrentIndex(0);
          setCurrentRep(1);
  ...
  return (
    <div className="min-h-screen bg-white text-[#171717] selection:bg-[#79ffe1] selection:text-black">
      ...
      <main className="mx-auto flex w-full max-w-5xl flex-col items-center justify-center px-10 pt-8 pb-8">
        {/* Player Card (Title removed as redundant) */}
        <div className="relative mb-8 w-full max-w-3xl overflow-hidden rounded-xl bg-white px-10 pt-10 pb-5 shadow-[0_0_0_1px_rgba(0,0,0,0.08),0_2px_4px_rgba(0,0,0,0.04)] before:absolute before:top-0 before:left-0 before:right-0 before:h-0.5 before:bg-[#171717]">
          ...
          <div className="mt-12 w-full flex flex-col gap-4">
            {/* Overall Progress */}
            <div className="mb-6">
              <div className="flex justify-between text-[10px] font-bold uppercase tracking-wider text-[#999999] mb-1.5">
            ...
            <div>
              <div className="flex justify-between text-[10px] font-bold uppercase tracking-wider text-[#999999] mb-1.5">
                <span>{isPlaying ? (phase === "jp" ? "Japanese" : "English") : "Japanese"} Phase Progress</span>
          ...
          <div className="mt-8 flex flex-col items-center gap-4">
            <div className="flex justify-center gap-4">
              <button 
                ...
                disabled={!isPlaying}
                className="flex items-center gap-2 rounded-full bg-white px-4 py-2 text-sm font-medium shadow-[0_0_0_1px_rgba(0,0,0,0.08)] hover:bg-[#fafafa] disabled:opacity-40 disabled:cursor-not-allowed disabled:hover:bg-white transition-opacity"
              >
